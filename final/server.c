...
int main(int argc, char ** argv) {
	...
	strcpy(host,argv[1]);
	strcpy(url,argv[2]);
	sprintf(message_fmt,"HEAD %s HTTP/1.0\r\n\r\n",argv[2]);
	...
	for(i=0;i<nodeCount;i++) {
		...
		if(byteTotal-current>=connectedNodes[i]->cap){
			...
			bytesPending -= connectedNodes[i]->cap;
		}
		else{
			connectedNodes[i]->byteLength = byteTotal-current+1;
			current = byteTotal;
			bytesPending = 0;
		}
	...
	int bytesReceived = 0;
		...
		FILE * fstream;
		char name[100];
		fstream = fopen("./bin/output.bin","wb");

	printf("Waiting for data. %d bytes still to be assigned.\n",bytesPending);
	while(bytesReceived < byteTotal-10) {
		...
		read(newSock,&incoming,sizeof(ClientNode));
		...
		int bytes=1;
		while(bytes>0) {
			bytes = read(newSock,response,sizeof(response));
			bytesReceived +=bytes;
			fwrite(response,bytes,1,fstream);
		}
		...
		if(bytesPending) {
			...
			printf("Assigning pending bytes to free nodes.\n");

			closing = 0;
			write(newSock,&closing,sizeof(closing));

			nodeCount++;
			incoming.id = nodeCount;
			
			incoming.bytesFrom = current;
			if(byteTotal-current>=incoming.cap){
				incoming.byteLength = incoming.cap;
				current += incoming.cap;
				bytesPending -= incoming.cap;
			}
			else{
				incoming.byteLength = byteTotal-current+1;
				current = byteTotal;
				bytesPending = 0;
			}

			write(newSock, &incoming,sizeof(ClientNode));
